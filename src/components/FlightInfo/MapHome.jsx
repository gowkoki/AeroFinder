...
const MapHome = (props) => {
  const { onClose, visible, gateLatitude, gateLongitude, gateAddress } = props;
  let userMarker;
  ...
  useEffect(() => {
    if (gateLatitude && gateLongitude) {
      ...
      if (location === "Gate") {
        infoElement.style.display = "none"; // Hide the infoElement
      }

      const textField = document.createElement("input");
      textField.value = gateLocation;
      textField.classList.add("bg-white", "rounded", "p-2", "w-24");
  ...
  }, [gateLatitude, gateLongitude]);

  if (!visible) return null;

  const containerSize = window.innerWidth > 768 ? "80%" : "90%";

  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center">
      <div className="bg-white p-2 rounded" style={{ width: containerSize }}>
