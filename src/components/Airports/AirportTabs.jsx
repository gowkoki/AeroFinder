...
const AirportTabs = (props) => {
  ...
  const fetchDepartureData = async () => {
    try {
      ...
      let depArrEndpoint = `http://localhost:8000/airport/departure/arrival?airport=${airport}`;
      let weatherEndpoint = `http://localhost:8000/airport/weather?lat=${airportData.location.lat}&lon=${airportData.location.lon}`;
