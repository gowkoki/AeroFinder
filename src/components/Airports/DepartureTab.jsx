...
const DepartureTab = ({ departureData }) => {
  ...
  const currentItems = departureData.slice(indexOfFirstItem, indexOfLastItem);

  return (
    <div className=" mt-5 relative overflow-x-auto shadow-md sm:rounded-lg">
      {/* Table for displaying departure data */}
        ...
        <tbody>
          {currentItems.map((flight) => (
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
              ...
              <td className="px-6 py-4">
                <p>{flight.arrival.airport.iata}</p>
                <p className="text-xs">{flight.arrival.airport.name}</p>
              </td>
      ...
      <div className="flex justify-center mt-4">
        <ReactPaginate
          previousLabel={"←"}
          nextLabel={"→"}
          pageCount={Math.ceil(departureData.length / itemsPerPage)}
